...
//			}
	...
	document.write('<p>tracking gif: <img src=\"' + returnUrl() + '\"></p>');
		...
		function populateFieldDefs() {
            ...
            $fp_fields = get_cache('sherlock/fp_fields', 86400, "SELECT * FROM fp_field WHERE active = 1");
		...
		function dbVersionCurrent($dbVersionExpected) {
			...
			$dbVersion = get_cache('sherlock/db_version', 86400, $query, 'get_var');
		...
		function getClientRecords() {
			$clientId = intval($this->getClientId());
			...
			$records = get_cache("sherlock/clientrecords/$clientId", 60, $query);
...
//
		...
		function loadFromSession($sessionId) {
			...
			$results = get_cache("sherlock/session/$sessionId", 60, $query);
